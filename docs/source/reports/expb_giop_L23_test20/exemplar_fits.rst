====================================
Exemplar fits — expb_giop_L23_test20
====================================
...
Every other figure in this report describes a **population** of retrievals. This page shows 10 individual fits from sweep ``expb_giop_L23_test20`` — the question an ocean-colour reader asks as soon as a summary statistic looks wrong: *show me a spectrum where it failed*.
...
**What these 10 fits are.** All 10 sit at or below the χ²ᵥ = 5 solution threshold. 6 sit *below* χ²ᵥ = 1, i.e. over-fit relative to the assumed noise. Recorded fit status across these observations and algorithms: ``ok`` 20.
...
.. figure:: closure_L23_17.png
   :width: 90%
   :alt: ``Rrs_obs − Rrs_model`` for observation ``17``, per algorithm, with each algorithm's χ²ᵥ in the legend.

   ``Rrs_obs − Rrs_model`` for observation ``17``, per algorithm, with each algorithm's χ²ᵥ in the legend.

Rrs closure — worst fit (obs 15)
--------------------------------

Closure residuals for the same fit as above, which is the view that shows *where* in the spectrum the model fails rather than by how much overall: a residual that is flat but offset is a different fault from one that swings sign across the green.

.. figure:: closure_L23_15.png
   :width: 90%
   :alt: ``Rrs_obs − Rrs_model`` for observation ``15``, per algorithm, with each algorithm's χ²ᵥ in the legend.

   ``Rrs_obs − Rrs_model`` for observation ``15``, per algorithm, with each algorithm's χ²ᵥ in the legend.
